...
import { getValidCalendarToken, calendarFetch } from '../../lib/google-calendar'
...
export default defineEventHandler(async (event) => {
    ...
    console.log(`[Calendar Webhook] state=${resourceState} channel=${channelId} resource=${resourceId}`)
    ...
    if (resourceState === 'exists') {
        ...
        try {
            ...
            if (!emp?.calendarTokens) {
                console.warn('[Calendar Webhook] No employee found for channel:', channelId)
                return { ok: true }
            }
            ...
            if (emp.calendarSyncToken) {
                try {
                    ...
                    console.log(`[Calendar Webhook] Synced ${data.items?.length || 0} changed events for ${emp.calendarEmail}`)
                } catch (syncError: any) {
                    ...
                    if (syncError.message?.includes('410')) {
                        ...
                        console.warn('[Calendar Webhook] SyncToken expired, cleared for next full sync')
                    }
                }
            }
        } catch (e) {
            console.error('[Calendar Webhook] Error processing notification:', e)
        }
    }
