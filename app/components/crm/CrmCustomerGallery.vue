...
const apiBase = computed(() => props.apiPrefix || '/api/pipeline')
...
async function handleFileSubmit(event: Event) {
  ...
  try {
    ...
    if (uploadedImages.length > 0) {
      ...
      const updateRes = await $fetch<any>(`${apiBase.value}/${props.customer._id}`, {
        method: 'PUT',
...
async function confirmDelete() {
  ...
  if (info.type === 'single') {
    ...
    try {
      const res = await $fetch<any>(`${apiBase.value}/${props.customer._id}`, { method: 'PUT', body: { gallery: currentGallery } })
      if (res.success) {
...
async function updateCaption() {
  ...
  try {
    const res = await $fetch<any>(`${apiBase.value}/${props.customer._id}`, {
      method: 'PUT',
  ...
  <div class="h-full w-full">
    ...
    <div v-if="(!customer.gallery || customer.gallery.length === 0) && uploadQueue.length === 0" class="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-border rounded-xl bg-card">
      ...
      <p class="text-sm text-muted-foreground mb-4 max-w-sm">
        Upload photos of the space, before/after shots, or material references.
      </p>
      <button class="text-sm font-bold text-primary hover:underline transition-all hover:text-primary/80" @click="triggerUpload">
        Browse files
      </button>
    </div>
      ...
      >
        ...
        <div v-if="!selectionMode" class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3">
          ...
          <div class="flex flex-col">
            <p v-if="img.caption" class="text-xs text-white font-semibold line-clamp-2 mb-1 drop-shadow-md leading-tight">
              {{ img.caption }}
            </p>
          ...
          <template v-else>
            <div class="relative size-10 flex items-center justify-center">
              ...
              <svg class="absolute inset-0 size-full -rotate-90" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
                <circle cx="18" cy="18" r="16" fill="none" class="stroke-primary/20" stroke-width="3" />
                <circle cx="18" cy="18" r="16" fill="none" class="stroke-primary transition-all duration-300" stroke-width="3" stroke-dasharray="100" :stroke-dashoffset="100 - item.progress" stroke-linecap="round" />
              </svg>
          ...
          <div class="w-full border-t border-border/50 bg-background/80 p-4 shrink-0 backdrop-blur-md">
            <div v-if="selectedImage" class="flex flex-col sm:flex-row items-center gap-6 w-full">
              ...
              <div class="flex items-center justify-end w-full sm:w-auto shrink-0 pb-4 sm:pb-0">
                <button class="h-10 px-6 text-xs uppercase tracking-wider font-bold text-red-500 bg-red-500/10 hover:bg-red-500 hover:text-white rounded-lg transition-all flex items-center justify-center gap-2 shadow-sm" @click="removeImage(selectedImage.index)">
