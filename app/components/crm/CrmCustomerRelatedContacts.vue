  ...
  <div class="flex flex-col h-full space-y-4">
    <div v-if="isEditing" class="bg-muted/30 border border-border/50 rounded-xl p-4 sm:p-5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      <div class="space-y-1.5">
        <label class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center justify-between">
          Emails
          <button type="button" class="text-[10px] text-primary hover:underline flex items-center gap-1" @click="addEmail">
        ...
        <div v-for="(email, i) in form.emails" :key="i" class="flex items-center gap-2 mb-2">
          ...
          <button type="button" class="shrink-0 p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors" @click="removeEmail(i)">
      ...
      <div class="space-y-1.5">
        <label class="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center justify-between">
          Phones
          <button type="button" class="text-[10px] text-primary hover:underline flex items-center gap-1" @click="addPhone">
            <Icon name="i-lucide-plus" class="size-3" /> Add Phone
          </button>
        </label>
        <div v-for="(phone, i) in form.phones" :key="i" class="flex items-center gap-2 mb-2">
          <input :value="form.phones[i]" type="text" class="w-full bg-background border border-input rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all" placeholder="(xxx) xxx-xxxx" @input="onPhoneInput(i, $event)">
          <button type="button" class="shrink-0 p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-colors" @click="removePhone(i)">
            <Icon name="i-lucide-trash-2" class="size-4" />
          </button>
        </div>
      </div>
      ...
      <div class="space-y-1.5">
        ...
        <textarea v-model="form.address" rows="2" class="w-full bg-background border border-input rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all resize-none" placeholder="123 Main St..." />
      </div>

      <div class="flex items-center justify-end gap-2 pt-2">
        <button type="button" class="px-4 py-2 text-sm font-bold text-muted-foreground hover:text-foreground transition-colors" :disabled="isLoading" @click="isEditing = false">
          Cancel
        </button>
        <button type="button" class="px-4 py-2 bg-primary text-primary-foreground text-sm font-bold rounded-lg shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all flex items-center justify-center gap-2" :disabled="isLoading" @click="saveContact">
    ...
    <div v-else class="flex-1 flex flex-col items-center justify-center text-center py-8">
      ...
      <p class="text-xs text-muted-foreground max-w-[200px]">
        Add associated contacts like property managers or assistants here.
      </p>
    </div>
  </div>
</template>
