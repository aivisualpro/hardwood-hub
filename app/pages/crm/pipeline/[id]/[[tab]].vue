...
async function fetchAllData(email: string, phone: string) {
  ...
  try {
    ...
    if (res.success) allSubmissions.value = res.data || []
  } catch {
    ...
    <ClientOnly>
      <Teleport to="#header-toolbar">
        <div class="flex items-center gap-2">
          ...
          <div v-if="customer" class="relative" :class="activeDropdown === 'stage' ? 'z-50' : ''">
            ...
            <div v-if="activeDropdown === 'stage'" class="absolute left-0 mt-1 top-full w-[200px] bg-card/95 backdrop-blur-md border border-border rounded-lg shadow-xl z-50 flex flex-col ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-150">
              <div class="p-2 border-b border-border/50">
                <input ref="stageSearchInput" type="text" v-model="stageSearch" placeholder="Search status..." class="w-full bg-background border border-border/50 rounded px-2 py-1.5 text-xs outline-none focus:ring-1 focus:ring-primary font-medium" @click.stop />
              </div>
          ...
          <button @click="showEditCustomer = true" class="inline-flex items-center justify-center gap-2 h-8 sm:h-9 px-3 sm:px-4 rounded-lg bg-primary text-primary-foreground text-xs sm:text-sm font-bold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">
            <Icon name="i-lucide-pencil" class="size-3.5" />
            <span class="hidden sm:inline">Edit</span>
          </button>
          <button @click="showDeleteConfirm = true" class="inline-flex items-center justify-center gap-2 h-8 sm:h-9 px-3 sm:px-4 rounded-lg bg-destructive/10 text-destructive text-xs sm:text-sm font-bold hover:bg-destructive/20 transition-all">
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          ...
          <div v-if="customer" class="px-5 py-4 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Email</p>
              ...
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Phone</p>
            ...
            <div v-if="customer.address || customer.city">
              <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Address</p>
              <p class="text-xs text-foreground/80 font-medium leading-relaxed">{{ [customer.address, customer.city, customer.state, customer.zip].filter(Boolean).join(', ') || '—' }}</p>
            </div>
            <!-- Assignment -->
            <div class="grid grid-cols-2 gap-3">
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Assigned To</p>
                <p class="text-xs font-semibold text-foreground">{{ customer.assignedTo || '—' }}</p>
              </div>
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Project Manager</p>
                <p class="text-xs font-semibold text-foreground">{{ customer.projectAssignedTo || '—' }}</p>
              </div>
            </div>
            <!-- Financial -->
            <div class="grid grid-cols-2 gap-3">
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Total Estimate</p>
                <p class="text-base font-black tabular-nums text-foreground">{{ customer.totalEstimate ? '$' + customer.totalEstimate.toLocaleString() : '—' }}</p>
              </div>
              <div>
                <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Duration</p>
                <p class="text-xs font-semibold text-foreground">{{ customer.estimatedProjectDuration || '—' }}</p>
              </div>
            </div>
            <!-- Tags -->
            ...
            <div v-if="customer.notes">
              <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Notes</p>
              <p class="text-xs text-foreground/70 leading-relaxed whitespace-pre-wrap bg-muted/30 rounded-lg p-3 border border-dashed">{{ customer.notes }}</p>
            </div>
            <!-- Key Dates -->
            <div class="pt-1 border-t border-border/50">
              <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Key Dates</p>
              <div class="grid grid-cols-2 gap-2">
                <div v-if="customer.initialContactDate">
                  <p class="text-[9px] text-muted-foreground uppercase tracking-wider">Initial Contact</p>
                  <p class="text-[11px] font-semibold">{{ formatDate(customer.initialContactDate) }}</p>
                </div>
                <div v-if="customer.estimateSentOn">
                  <p class="text-[9px] text-muted-foreground uppercase tracking-wider">Estimate Sent</p>
                  <p class="text-[11px] font-semibold">{{ formatDate(customer.estimateSentOn) }}</p>
                </div>
                <div v-if="customer.dateApproved">
                  <p class="text-[9px] text-muted-foreground uppercase tracking-wider">Approved</p>
                  <p class="text-[11px] font-semibold">{{ formatDate(customer.dateApproved) }}</p>
                </div>
                <div v-if="customer.woodOrderDate">
                  <p class="text-[9px] text-muted-foreground uppercase tracking-wider">Wood Ordered</p>
                  <p class="text-[11px] font-semibold">{{ formatDate(customer.woodOrderDate) }}</p>
                </div>
                <div v-if="customer.lastFollowUpSentOn">
                  <p class="text-[9px] text-muted-foreground uppercase tracking-wider">Last Follow-up</p>
                  <p class="text-[11px] font-semibold">{{ formatDate(customer.lastFollowUpSentOn) }}</p>
                </div>
              </div>
            </div>
            <!-- Meta -->
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/30 flex items-center justify-between gap-2 shrink-0">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-bold text-foreground">Related Contacts</h3>
            </div>
            <button
          ...
          <div class="px-5 py-4">
            <CrmCustomerRelatedContacts ref="relatedContactsRef" v-if="customer" :customer="customer" @updated="onCustomerUpdated" />
          </div>
        </div>
      </div>
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/30 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-bold text-foreground">Related Quotes</h3>
            </div>
          ...
          <div v-else-if="relatedQuotes.length === 0" class="flex-1 flex flex-col items-center justify-center py-8 text-center">
            ...
            <p class="text-xs text-muted-foreground">No quote submissions</p>
          </div>
          <div v-else class="divide-y divide-border/50">
            <div v-for="item in relatedQuotes" :key="item._id" class="flex items-center gap-3 px-5 py-3 hover:bg-muted/20 transition-colors">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-foreground truncate">{{ item.name || 'Anonymous' }}</p>
                <p class="text-[10px] text-muted-foreground">{{ formatDate(item.fields?.meetingScheduled?.startTime || item.dateSubmitted) }}</p>
              </div>
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/30 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-bold text-foreground">Related Estimates</h3>
            </div>
          ...
          <div v-else-if="relatedEstimates.length === 0" class="flex-1 flex flex-col items-center justify-center py-8 text-center">
            ...
            <p class="text-xs text-muted-foreground">No estimate submissions</p>
          </div>
          <div v-else class="divide-y divide-border/50">
            <div v-for="item in relatedEstimates" :key="item._id" class="flex items-center gap-3 px-5 py-3 hover:bg-muted/20 transition-colors">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-foreground truncate">{{ item.name || 'Anonymous' }}</p>
                <p class="text-[10px] text-muted-foreground">{{ formatDate(item.dateSubmitted) }}</p>
              </div>
              <span class="text-[10px] font-bold px-2 py-0.5 rounded-full capitalize shrink-0" :class="submissionStatusClass(item.status)">{{ item.status }}</span>
            </div>
          </div>
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/30 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-sm font-bold text-foreground">Related Contracts</h3>
            </div>
            <div class="flex items-center gap-2">
              ...
              <button v-if="customer" @click="contractFormDialog?.openForCustomer(customer)" class="inline-flex items-center gap-1 h-7 px-2.5 rounded-lg bg-primary text-primary-foreground text-[11px] font-bold hover:bg-primary/90 transition-all">
                <Icon name="i-lucide-plus" class="size-3" />
                Add Contract
              </button>
            </div>
          ...
          <div v-else-if="customerContracts.length === 0" class="flex-1 flex flex-col items-center justify-center py-8 text-center">
            ...
            <p class="text-xs text-muted-foreground">No contracts on file</p>
          </div>
      ...
      <div class="w-[30%] min-w-0 flex flex-col gap-3 px-4 lg:px-5 py-4 overflow-y-auto">
        ...
        <div class="bg-card rounded-2xl border shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/30 flex items-center justify-between gap-3 shrink-0">
            <div class="flex items-center gap-2 shrink-0">
              ...
              <h3 class="text-sm font-bold text-foreground">Project Gallery</h3>
            ...
            <div v-if="customer && galleryRef" class="flex items-center gap-1.5">
              <template v-if="customer.gallery?.length > 0">
                <button v-if="!galleryRef.selectionMode" @click="galleryRef.toggleSelectionMode()" class="inline-flex items-center justify-center h-7 px-2.5 rounded-lg bg-muted text-foreground text-[11px] font-bold hover:bg-muted/80 transition-all">
                  Select
                </button>
                <template v-else>
                  <button @click="galleryRef.removeSelectedImages()" :disabled="galleryRef.selectedIndices.size === 0" class="inline-flex items-center gap-1 h-7 px-2.5 rounded-lg bg-destructive text-white text-[11px] font-bold hover:bg-destructive/90 transition-all disabled:opacity-50">
                    <Icon name="i-lucide-trash-2" class="size-3" />
                    Delete ({{ galleryRef.selectedIndices.size }})
                  </button>
                  <button @click="galleryRef.toggleSelectionMode()" class="inline-flex items-center justify-center h-7 px-2.5 rounded-lg border border-border text-foreground text-[11px] font-bold hover:bg-muted transition-all">
                    Cancel
                  </button>
                </template>
              </template>
              <button v-if="!galleryRef.selectionMode" @click="galleryRef.triggerUpload()" :disabled="galleryRef.isUploading" class="inline-flex items-center gap-1 h-7 px-2.5 rounded-lg bg-primary text-primary-foreground text-[11px] font-bold hover:bg-primary/90 transition-all disabled:opacity-50">
        ...
        <div class="bg-card rounded-2xl border border-dashed shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b bg-muted/20 flex items-center gap-2 shrink-0">
            ...
            <h3 class="text-sm font-bold text-muted-foreground/40">Coming Soon</h3>
          </div>
          <div class="flex-1 flex flex-col items-center justify-center text-center gap-2">
            ...
            <p class="text-xs text-muted-foreground/40 font-medium">More content coming here</p>
          </div>
        </div>
