...
async function parsePdf() {
  ...
  try {
    ...
    const res = await $fetch<{ success: boolean, data: any }>('/api/estimates/templates/parse-pdf', {
      method: 'POST',
      body: {
        pdfBase64: base64,
...
const { canCreate, canUpdate, canDelete } = usePermissions('/crm/estimates')

// Initial header state
setHeader({
  title: 'Estimates',
  icon: 'i-lucide-file-text',
  description: 'Manage estimates and templates',
})
...
if (!route.params.tab) {
  navigateTo('/crm/estimates/list', { replace: true })
}
...
// ─── Templates State ─────────────────────────────────────
const templates = useState<EstimateTemplate[]>('crm-estimate-templates', () => [])
const loadingTemplates = ref(false)
const selectedTemplate = ref<EstimateTemplate | null>(null)
...
watch(() => showEditor.value, (isEditing) => {
  if (isEditing) {
    ...
    setHeader({
      ...
      description: 'Customize estimate layout and variables',
    })
  }
  else {
...
async function performAutoSave() {
  ...
  try {
    await $fetch(`/api/estimates/templates/${selectedTemplate.value._id}`, {
      method: 'PUT',
...
async function fetchTemplates() {
  ...
  try {
    const res = await $fetch<{ success: boolean, data: EstimateTemplate[] }>('/api/estimates/templates')
...
function openTemplateEditor(template: EstimateTemplate) {
...
function openNewTemplate() {
  ...
  templateForm.value = {
    name: '',
    description: '',
    content: '<p>Start writing your estimate template...</p>',
    category: 'General',
    variables: [
      { key: 'estimate_number', label: 'Estimate Number', type: 'text', defaultValue: '', required: true, scope: 'template' },
    ],
...
async function saveTemplate() {
  ...
  try {
    if (selectedTemplate.value) {
      ...
      const res = await $fetch<{ success: boolean, data: any }>('/api/estimates/templates', {
        method: 'POST',
        body: templateForm.value,
      })
      if (res.data?._id) {
        selectedTemplate.value = res.data as EstimateTemplate
      }
...
function addVariable() {
  templateForm.value.variables.push({ key: '', label: '', type: 'text', defaultValue: '', required: false, scope: 'template', _isNew: true })
}
...
const estimates = useState<EstimateRecord[]>('crm-estimates', () => [])
const loadingEstimates = ref(true)
const searchQuery = ref('')

async function fetchEstimates() {
  loadingEstimates.value = true
  try {
    const res = await $fetch<{ success: boolean, data: EstimateRecord[] }>('/api/estimates', {
      params: searchQuery.value ? { search: searchQuery.value } : {},
    })
    estimates.value = res.data || []
  }
  catch (e: any) {
    toast.error('Failed to load estimates', { description: e?.message })
  }
  finally {
...
function openCreateModal() {
  estimateFormDialog.value?.openCreateModal()
}

function openEditEstimate(ct: any) {
  estimateFormDialog.value?.openEditEstimate(ct)
}
...
if (import.meta.client) {
  Promise.all([fetchTemplates(), fetchCompanyProfile(), fetchEstimates()])
}
...
<template>
  <div class="space-y-0 -mt-4 lg:-mt-6">
        ...
        <div class="flex items-center justify-start overflow-x-auto no-scrollbar w-full">
          <div class="flex items-center gap-0.5 min-w-max">
            <button
              ...
              @click="navigateTo(`/crm/estimates/${tab.id}`); showEditor = false"
            >
              <Icon :name="tab.icon" class="size-3.5 sm:size-4" />
              {{ tab.label }}
              <div
                v-if="tab.id === 'list' && estimates.length > 0"
      ...
      <div class="flex-1 overflow-hidden">
        ...
        <div v-if="activeTab === 'list'" class="h-full overflow-y-auto">
          <CrmEstimatesTable
            ...
            :is-loading="loadingEstimates"
            @refresh="fetchEstimates"
            @edit="openEditEstimate"
          />
        </div>
        ...
        <template v-if="activeTab === 'templates'">
          ...
          <div v-if="showEditor" class="h-full">
            <div class="h-full overflow-hidden grid grid-cols-1 xl:grid-cols-[1fr_450px] gap-0 border border-border/50 rounded-xl bg-card shadow-sm">
              ...
              <!-- RIGHT: Variables (system variables only, no client scope) -->
              <div class="flex flex-col overflow-hidden">
                <div class="flex-1 overflow-y-auto">
                  ...
                  <div class="divide-y divide-border/30">
                    <div v-for="(v, idx) in templateForm.variables" :key="idx">
                      <div class="p-2.5 hover:bg-muted/20 transition-colors group flex items-center gap-2">
                        ...
                        <div class="flex-1 min-w-0 relative group">
                          <input v-model="v.label" :readonly="v.key === 'estimate_number'" placeholder="Variable Name" class="w-full text-[10px] font-semibold text-amber-600 dark:text-amber-400 bg-amber-500/5 hover:bg-amber-500/10 border border-amber-500/20 focus:border-amber-500/50 rounded px-2 py-1.5 outline-none transition-all pr-12" :class="{ 'opacity-80 cursor-default': v.key === 'estimate_number' }" @input="syncVariableKeyAndContent(v)">
                        ...
                        <div class="flex items-center gap-1 shrink-0 px-1" title="Required">
                          <input :id="`req-${idx}`" v-model="v.required" type="checkbox" :disabled="v.key === 'estimate_number'" class="size-3.5 rounded border-border text-primary focus:ring-primary cursor-pointer disabled:opacity-50">
                          <label :for="`req-${idx}`" class="text-[10px] text-muted-foreground cursor-pointer font-medium select-none">Req</label>
                        </div>
                        <div class="flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" :class="{ 'invisible pointer-events-none': v.key === 'estimate_number' }">
                        ...
                        <button class="size-6 rounded flex items-center justify-center text-muted-foreground/50 hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100 shrink-0" :class="{ 'invisible pointer-events-none': v.key === 'estimate_number' }" @click="removeVariable(idx)">
                    ...
                    <div v-if="!templateForm.variables.length" class="p-4 text-center">
                      <p class="text-xs text-muted-foreground">
                        Add variables that will be filled during estimate creation
                      </p>
                    </div>
                  </div>
                  ...
                  <div class="bg-muted/10 border-t border-border/50 p-3 flex flex-col gap-2">
                    ...
                    >
                      ...
                      <!-- Invisible placeholders to match variable row width -->
          ...
          <div v-else class="h-full overflow-y-auto">
            ...
            <div v-else-if="templates.length === 0" class="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-border/60 rounded-2xl bg-muted/5">
              ...
              <p class="text-muted-foreground max-w-sm px-4 text-sm mb-4">
                Create your first estimate template to get started.
              </p>
    ...
    <CrmEstimateFormDialog ref="estimateFormDialog" @saved="fetchEstimates" />
          ...
          <div v-if="pdfPreviewStep === 'preview' && pdfParsed" class="divide-y">
            ...
            <div class="p-5 space-y-3">
              ...
              <div v-else class="space-y-1.5 max-h-48 overflow-y-auto">
                ...
                >
                  <span
                    class="px-2 py-0.5 rounded text-[10px] font-mono font-bold shrink-0 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20"
                  >{{ v.key }}</span>
