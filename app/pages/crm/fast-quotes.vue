...
setHeader({
  title: 'Fast Quotes',
  icon: 'i-lucide-zap',
  description: 'Manage fast quote requests from potential customers',
})
...
<template>
  <div class="space-y-6">
    ...
    <!-- Hero Banner -->
    <div class="relative overflow-hidden rounded-2xl border bg-gradient-to-br from-amber-500/10 via-card to-card p-6">
      <div class="absolute -right-8 -top-8 w-32 h-32 rounded-full bg-amber-500/5 blur-2xl" />
      <div class="absolute -right-4 -bottom-4 w-24 h-24 rounded-full bg-amber-400/5 blur-xl" />
      <div class="relative flex items-center gap-4">
        <div class="w-14 h-14 rounded-2xl bg-amber-500/15 flex items-center justify-center ring-1 ring-amber-500/20 shadow-lg shadow-amber-500/10">
          <Icon name="i-lucide-zap" class="size-7 text-amber-500" />
        </div>
        <div>
          <h2 class="text-xl font-bold text-foreground font-display">Fast Quote Requests</h2>
          <p class="text-sm text-muted-foreground mt-0.5">
            Submissions from "Get a FAST Quote" form
            <span v-if="totalItems > 0" class="text-amber-500 font-semibold">· {{ totalItems }} total</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="relative">
        <CrmSubmissionsTable
          ...
          empty-title="No fast quote requests yet"
