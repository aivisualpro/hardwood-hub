...
const employeeBonusData = computed<EmpBonusData[]>(() => {
  if (!tree.value.length || !employees.value.length)
    return []
    ...
    const categories = tree.value.map((cat) => {
      ...
      const subCategories = cat.subCategories.map((sub) => {
        ...
        const skillStatuses: SkillStatusItem[] = sub.skills.map((sk) => {
          ...
          if (h) {
            if (h.currentSkillLevel === 'Mastered')
              status = 'mastered'
            else if (h.currentSkillLevel === 'Proficient')
              status = 'proficient'
            else status = 'needs'
          }
        ...
        for (const sk of skillStatuses) {
          if (sk.status !== 'unreviewed')
            subAssessed++
          if (sk.status === 'mastered')
            subMastered++
          else if (sk.status === 'proficient')
            subProficient++
          else if (sk.status === 'needs')
            subNeeds++
        }
...
const filteredData = computed(() => {
  ...
  data = [...data].sort((a, b) => {
    let cmp = 0
    if (sortBy.value === 'bonus')
      cmp = a.totalBonus - b.totalBonus
    else if (sortBy.value === 'progress')
      cmp = a.totalAssessed - b.totalAssessed
...
const topEarner = computed(() => {
  if (!employeeBonusData.value.length)
    return null
...
const filteredMaxPotentialCategories = computed(() => {
  ...
  if (!q)
    return maxPotentialResult.value.categories
...
function toggleRow(empId: string) {
  if (expandedRows.value.has(empId))
    expandedRows.value.delete(empId)
...
function subStatus(sub: EmpBonusData['categories'][0]['subCategories'][0]): 'all-complete' | 'has-progress' | 'none' {
  if (sub.total === 0)
    return 'none'
  if (sub.ruleCompliant === sub.total)
    return 'all-complete'
  if (sub.assessed > 0)
    return 'has-progress'
  return 'none'
...
<template>
  <div class="space-y-4 sm:space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2.5 sm:gap-4">
      ...
      <div class="rounded-xl border border-amber-500/20 bg-gradient-to-br from-amber-500/10 to-amber-500/5 p-3 sm:p-5 relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-2 sm:p-3 opacity-15">
          <Icon name="i-lucide-trophy" class="size-14 sm:size-20 text-amber-500" />
        </div>
        <div class="relative z-10">
          <p class="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-amber-600/70 dark:text-amber-400/70">
            Total Payout
          </p>
          <p class="text-xl sm:text-3xl font-black text-amber-600 dark:text-amber-400 mt-1 sm:mt-2 tabular-nums">
            {{ fmt(totalBonusAllEmployees) }}
          </p>
          <p class="text-[9px] sm:text-[10px] text-amber-600/50 dark:text-amber-400/50 mt-0.5 sm:mt-1">
      ...
      <div class="rounded-xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 to-blue-500/5 p-3 sm:p-5 relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-2 sm:p-3 opacity-15">
          <Icon name="i-lucide-bar-chart-3" class="size-14 sm:size-20 text-blue-500" />
        </div>
        <div class="relative z-10">
          <p class="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-blue-600/70 dark:text-blue-400/70">
            Avg / Employee
          </p>
          <p class="text-xl sm:text-3xl font-black text-blue-600 dark:text-blue-400 mt-1 sm:mt-2 tabular-nums">
            {{ fmt(avgBonus) }}
          </p>
          <p class="text-[9px] sm:text-[10px] text-blue-600/50 dark:text-blue-400/50 mt-0.5 sm:mt-1">
      ...
      <div class="rounded-xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 p-3 sm:p-5 relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-2 sm:p-3 opacity-15">
          <Icon name="i-lucide-crown" class="size-14 sm:size-20 text-emerald-500" />
        </div>
        <div class="relative z-10">
          <p class="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-emerald-600/70 dark:text-emerald-400/70">
            Top Earner
          </p>
          <p class="text-sm sm:text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1 sm:mt-2 truncate">
            {{ topEarner?.employee.employee || '—' }}
          </p>
          <p class="text-[9px] sm:text-[10px] text-emerald-600/50 dark:text-emerald-400/50 mt-0.5 sm:mt-1">
            {{ topEarner ? fmt(topEarner.totalBonus) : '$0.00' }}
          </p>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 to-violet-500/5 p-3 sm:p-5 relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-2 sm:p-3 opacity-15">
          <Icon name="i-lucide-layers" class="size-14 sm:size-20 text-violet-500" />
        </div>
        <div class="relative z-10">
          <p class="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-violet-600/70 dark:text-violet-400/70">
            Categories
          </p>
          <p class="text-xl sm:text-3xl font-black text-violet-600 dark:text-violet-400 mt-1 sm:mt-2 tabular-nums">
            {{ tree.length }}
          </p>
          <p class="text-[9px] sm:text-[10px] text-violet-600/50 dark:text-violet-400/50 mt-0.5 sm:mt-1">
            {{ totalSystemSkills }} skills
          </p>
        </div>
      </div>
      ...
      >
        <div class="absolute -inset-1 bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-fuchsia-500/20 blur-xl opacity-50 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
        ...
        <div class="relative z-10">
          ...
          <p class="text-xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500 mt-1 sm:mt-2 tabular-nums drop-shadow-sm">
            {{ fmt(maxPotentialBonus) }}
          </p>
          <p class="text-[9px] sm:text-[10px] text-pink-600/70 dark:text-pink-400/70 mt-0.5 sm:mt-1 font-medium">
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                ...
                <button title="View Profile" class="text-muted-foreground hover:text-primary transition-colors flex items-center justify-center p-1 rounded-md hover:bg-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/20" @click.stop="navigateTo(`/my-profile?employee=${row.employee._id}`)">
                      ...
                      <div class="divide-y divide-border/15">
                        ...
                        >
                          ...
                          <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex items-center gap-4 sm:gap-5 text-xs">
                              <div v-if="sub.mastered" class="text-center">
                                <p class="font-semibold text-emerald-500 tabular-nums">
                                  {{ sub.mastered }}
                                </p>
                              ...
                              <div v-if="sub.proficient" class="text-center">
                                <p class="font-semibold text-blue-500 tabular-nums">
                                  {{ sub.proficient }}
                                </p>
                                <p class="text-[9px] text-muted-foreground">
                                  Proficient
                              ...
                              <div v-if="sub.needs" class="text-center">
                                <p class="font-semibold text-amber-500 tabular-nums">
                                  {{ sub.needs }}
                                </p>
                                <p class="text-[9px] text-muted-foreground">
                                  Needs Imp.
                          ...
                          <div v-if="sub.rulesToUse && sub.rulesToUse.length" class="space-y-1.5">
                            <div
                              v-for="(rule, ri) in sub.rulesToUse" :key="ri" class="flex items-center gap-2.5 px-3 py-2 rounded-lg border transition-colors"
                              :class="(() => {
                            ...
                            >
                              ...
                              <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 rounded-lg bg-popover border border-border shadow-xl opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none z-20 min-w-[220px] max-w-[360px]">
                                ...
                                <div class="px-3 py-1.5 border-b border-border/50">
                                  ...
                                  <p class="text-[9px] capitalize" :class="sk.status === 'mastered' ? 'text-emerald-500' : sk.status === 'proficient' ? 'text-blue-500' : sk.status === 'needs' ? 'text-amber-500' : 'text-muted-foreground'">
                                    {{ sk.status === 'needs' ? 'Needs Improvement' : sk.status }}
                                  </p>
                                </div>
                                <!-- Reviewer list -->
  ...
  <Dialog v-model:open="showMaxBonusModal">
    <DialogContent class="sm:max-w-2xl bg-gradient-to-b from-card to-background p-0 overflow-hidden border-pink-500/20 shadow-[0_0_50px_-12px_rgba(236,72,153,0.25)]">
      ...
      <div class="relative px-6 py-8 overflow-hidden bg-gradient-to-br from-pink-500/10 via-purple-500/5 to-transparent border-b border-pink-500/10">
        <div class="absolute -inset-1 bg-gradient-to-r from-pink-500/20 via-purple-500/20 to-fuchsia-500/20 blur-2xl opacity-50 pointer-events-none" />
          ...
          >
            <div v-for="sub in cat.subCategories" :key="sub.name" class="rounded-xl border border-border/50 bg-muted/10 overflow-hidden shadow-sm hover:shadow-md hover:border-pink-500/30 transition-all duration-300">
              <div class="flex items-center gap-3 p-3 bg-muted/20 border-b border-border/50 group">
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-[13px] font-semibold truncate text-foreground">
                    {{ sub.name }}
                  </p>
              ...
              <div class="p-3 bg-background/50">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
                  <div v-for="sk in sub.skills" :key="sk.name" class="flex items-start gap-2">
                    <span class="size-1.5 rounded-full bg-pink-500/40 shrink-0 mt-1.5" />
      ...
      <div class="px-6 py-4 border-t border-border/50 bg-muted/10 flex items-center justify-between z-10 relative">
        ...
        <p class="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500 drop-shadow-sm tabular-nums">
          {{ fmt(maxPotentialBonus) }}
        </p>
      </div>
    </DialogContent>
  ...
  <Dialog v-model:open="showCustomBonusModal">
    <DialogContent class="sm:max-w-md">
      ...
      <DialogFooter>
        <Button variant="outline" :disabled="savingCustomBonus" @click="showCustomBonusModal = false">
          Cancel
        </Button>
        <Button class="bg-violet-500 hover:bg-violet-600 text-white" :disabled="savingCustomBonus" @click="saveCustomBonus">
