...
function statusColor(status: string) {
  ...
  if (s === 'done' || s === 'completed' || s === 'signed')
    return 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30'
  if (s === 'in progress' || s === 'in_progress' || s === 'in-progress' || s === 'rolling' || s === 'pending' || s === 'sent')
    return 'bg-blue-500/15 text-blue-400 border-blue-500/30'
  if (s === 'in-review' || s === 'in review')
    return 'bg-violet-500/15 text-violet-400 border-violet-500/30'
  if (s === 'todo' || s === 'to do' || s === 'draft')
...
const maxSqft = computed(() => {
  if (!stats.value?.production.trend.length)
    return 1
...
const maxReviews = computed(() => {
  if (!stats.value?.performance.trend.length)
    return 1
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
          ...
          >
            ...
            <div class="relative">
              ...
              <p class="text-[10px] sm:text-xs text-muted-foreground font-medium mt-0.5">
                {{ card.label }}
              </p>
            </div>
              ...
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p class="text-[10px] text-muted-foreground">
                    Square feet completed — last 30 days
                  </p>
                </div>
              </div>
            ...
            <div class="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-border/50">
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p class="text-[10px] text-muted-foreground">
                    {{ stats.employees.total }} team members
                  </p>
                </div>
              </div>
            ...
            <div v-else class="divide-y divide-border/30">
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p v-if="task.assigneeName" class="text-[9px] text-muted-foreground truncate">
                    {{ task.assigneeName }}
                  </p>
                </div>
                <span
            ...
            <div class="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-border/50">
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p class="text-[10px] text-muted-foreground">
                    {{ stats.performance.total.toLocaleString() }} total reviews
                  </p>
                </div>
              </div>
              <NuxtLink to="/hr/employees-bonus-report" class="text-[10px] sm:text-xs text-primary hover:underline font-medium">
                Bonus Report →
              </NuxtLink>
            </div>
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[9px] text-muted-foreground tabular-nums">
                    {{ p.entries }} entries · {{ p.hours }}h
                  </p>
                </div>
                <div class="text-right shrink-0">
                  <p class="text-xs font-bold text-amber-400 tabular-nums">
          ...
          <div class="rounded-xl border border-border/50 bg-card shadow-xs overflow-hidden">
            <div class="flex items-center gap-2 px-4 sm:px-5 py-3 sm:py-4 border-b border-border/50">
              ...
              <div>
                ...
                <p class="text-[10px] text-muted-foreground">
                  CRM leads by type · Contract status
                </p>
              </div>
            </div>
            ...
            <div class="p-4 sm:p-5">
              <!-- CRM by Type -->
              <p class="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 mb-2.5">
                CRM Submissions
              </p>
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[9px] text-muted-foreground truncate">
                    {{ prod.block1Category || 'No category' }}
                  </p>
                </div>
                <div class="text-right shrink-0">
                  <p v-if="prod.squareFeetCompleted" class="text-[10px] font-bold text-emerald-400 tabular-nums">
                    {{ prod.squareFeetCompleted }} sqft
                  </p>
                  <p v-if="prod.productionHours" class="text-[9px] text-muted-foreground tabular-nums">
                    {{ prod.productionHours }}h
                  </p>
                </div>
              </div>
            </div>
